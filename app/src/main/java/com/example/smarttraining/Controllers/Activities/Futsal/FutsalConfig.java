package com.example.smarttraining.Controllers.Activities.Futsal;
...
public class FutsalConfig extends AppCompatActivity {

    @BindView(R.id.futsal_config_period)
    Button period;
    @BindView(R.id.futsal_config_display_players)
    Button display_player;
    @BindView(R.id.futsal_config_nb_player)
    Button nb_player;
    @BindView(R.id.futsal_config_play)
    Button play;

    @State
    int period_value = 20;
    @State
    int nb_player_value = 4;
    @State
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_futsal_config);
    ...
    private void Launch_Match(){
        Intent otherActivity = new Intent(FutsalConfig.this, BadmintonMatch.class);
    ...
    private void Initialisation_equipes() {
        ...
        for (int i=0; i < NUMBER_PLAYER; i++){
            ...
            int resID_1 = getResources().getIdentifier("futsal_config_eq1_j" + (i + 1), "id", getPackageName());
            joueurs_equipe1[i] = findViewById(resID_1);
            joueurs_equipe1[i].setHint(player_index);

            int resID_2 = getResources().getIdentifier("futsal_config_eq2_j" + (i + 1), "id", getPackageName());
